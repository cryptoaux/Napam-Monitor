...
/*
 * Find submitted application numbers.
 */
...
function extractApplicationIds(html) {
  const ids = [];
  /*
   * Search the entire HTML instead of stopping
   * immediately when one index is missing.
   */
  const regex =
...
/*
 * Convert NAPAMS stage information into the
 * dashboard colors.
 */
...
/*
 * Check one application's status.
 */
...
/*
 * Save the data where GitHub Pages can access it.
 *
 * IMPORTANT:
 * Website file:
 *
...
async function main() {
  ...
  for (
    ...
    /*
     * Find the current stage so the website
     * can color the current-status badge.
     */
    const currentStage =
      stages.find(
        (stage) => {
